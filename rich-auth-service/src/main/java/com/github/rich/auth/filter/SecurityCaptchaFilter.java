...
public class SecurityCaptchaFilter extends GenericFilterBean {
    ...
    @Autowired
    public SecurityCaptchaFilter(CaptchaFilterConfig captchaFilterConfig, CaptchaValidateService defaultCaptchaValidateService, CaptchaValidateService smsCaptchaValidateService) {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if(requestMatcher.matches(request)){
            ...
            if (grantTypeOptional.isPresent()) {
                if (SecurityConstant.PASSWORD.toLowerCase().equals(grantTypeOptional.get().toLowerCase())&&captchaFilterConfig.getPassword()) {
                    // 拿到机器码
                ...
                if (SecurityConstant.MOBILE.toLowerCase().equals(grantTypeOptional.get().toLowerCase())&&captchaFilterConfig.getMobile()) {
                    // 拿到机器码 手机登录直接使用手机号码作为机器码
                    String machine = request.getParameter(MOBILE_PARAM);
                    // 拿到验证码
                    String captcha = request.getParameter(CAPTCHA_PARAM);
                    if(!smsCaptchaValidateService.validate(machine,captcha)){
                        throw new CaptchaCheckException("验证码错误");
                    }
                }
            }
